...
struct DocumentDetailView: View {
    ...
    @State private var documentId: Int = 0
    ...
    var body: some View {
        GeometryReader { geometry in
            ...
            VStack(spacing: 0) {
                ...
                Text("\(viewModel.documentDetail?.documentName ?? "Official SPOG's Documents")")
                ...
                ScrollView(.vertical, showsIndicators: false) {
                    VStack(spacing: 16) {
                        if let documentDetail = viewModel.documentDetail {
                            if let imagePath = documentDetail.image, !imagePath.isEmpty {
                            ...
                            HTMLText(html: documentDetail.title, color: "#2f2f2f", fontSize: 14)
                            
                            HTMLText(html: documentDetail.description, color: "#7e8492", fontSize: 10)
            ...
            .task {
                await Task { @MainActor in
                    viewModel.getSpogPresidentsMessageDetailsById(documentId: documentId)
                }.value
            }
            ...
            .toolbar {
                ...
                ToolbarItem(placement: .navigationBarTrailing) {
                    Button(action: {
                        if let documentDetail = viewModel.documentDetail {
                            let newBookmarkStatus = documentDetail.isBookmark ? 0 : 1
                            viewModel.bookmarkPost(itemType: "spog_document", itemId: documentDetail.id, isBookmark: newBookmarkStatus)
                        }
                    }) {
                        Image(systemName: (viewModel.documentDetail?.isBookmark ?? false) ? "bookmark.fill" : "bookmark")
