...
class AlertDetailViewModel: ObservableObject {
    ...
    @Published var alertDetails: SpogAlert?
    ...
    func getSpogAlertDetailsById(alertId:Int)  {
        LoadingIndicatorView.show()
        networkManager.executeServiceWithURL(urlString: WEBURL.spogAlertDetailsById + "/\(alertId)", postParameters: nil) { [weak self] response, error in
            ...
            if error == nil,let result = response {
                do {
                    ...
                    let response = try JSONDecoder().decode(AlertDetailResponse.self, from: jsonData)
