...
//  Valiant
//
//  Created by Kalyan Thakur on 07/02/26.
//
...
struct AlertDetailView: View {
    ...
    init(alertId:Int) {
        self._alertId = State(initialValue: alertId)
    }
        ...
        GeometryReader { geometry in
            ...
            VStack(spacing: 0) {
                ...
                ScrollView(.vertical, showsIndicators: false) {
                    VStack(spacing: 16) {
                        if let alertDetails = viewModel.alertDetails {
                            if let imagePath = alertDetails.image, !imagePath.isEmpty {
                            ...
                            HTMLText(html: alertDetails.title, color: "#2f2f2f", fontSize: 14)
                            
                            HTMLText(html: alertDetails.description, color: "#7e8492", fontSize: 10)

                            
                            // Date
                            Text(appSharedData.convertDateFormatFromForDate(fromFormat: "yyyy-MM-dd", toFormat: "MMM,dd,yyyy", stringDate: alertDetails.date))
            ...
            .task {
                await Task { @MainActor in
                    viewModel.getSpogAlertDetailsById(alertId: self.alertId)
                }.value
            }
