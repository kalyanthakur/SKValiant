...
struct EventDetailView: View {
    ...
    init(eventId:Int) {
        self._eventId = State(initialValue: eventId)
    }
        ...
        GeometryReader { geometry in
                ...
                ScrollView(.vertical, showsIndicators: false) {
                    VStack(spacing: 16) {
                        if let eventDetail = viewModel.eventDetail {
                            if let imagePath = eventDetail.image, !imagePath.isEmpty {
                            ...
                            HTMLText(html: eventDetail.title, color: "#2f2f2f", fontSize: 14)
                            
                            HTMLText(html: eventDetail.description, color: "#7e8492", fontSize: 10)
                            
                            let eventDate = appSharedData.convertDateFormatFromForDate(fromFormat: "yyyy-MM-dd", toFormat: "MMM dd, yyyy ", stringDate: eventDetail.date)
                            let eventTime = appSharedData.convertDateFormatFromForDate(fromFormat: "HH:MM:SS", toFormat: "hh:mm a", stringDate: eventDetail.time)
                            // Date
            ...
            .task {
                await Task { @MainActor in
                    viewModel.getSpogEventDetailsById(eventId: self.eventId)
                }.value
            }
            ...
            .toolbar {
                ...
                ToolbarItem(placement: .navigationBarTrailing) {
                    Button(action: {
                        if let eventDetail = viewModel.eventDetail {
                            let newBookmarkStatus = eventDetail.isBookmark ? 0 : 1
                            viewModel.bookmarkPost(itemType: "event", itemId: eventDetail.id, isBookmark: newBookmarkStatus)
                        }
                    }) {
                        Image(systemName: (viewModel.eventDetail?.isBookmark ?? false) ? "bookmark.fill" : "bookmark")
