...
struct SaucierScholarshipView: View {
    ...
    @State private var showScholarshipDetailView = false
    @State private var selectedSholarshipId = 0
        ...
        GeometryReader { geometry in
            ZStack(alignment: .bottomTrailing) {
                VStack(spacing: 0) {
                    ...
                    ScrollView(.vertical, showsIndicators: false) {
                        VStack(spacing: 16) {
                            ForEach(Array(viewModel.arrSholarships.enumerated()), id: \.element.id) { index, scholarship in
                                if index == 0 {
                                    FeatureSholarship(width: geometry.size.width-32, scholarship: scholarship)
                                        .onTapGesture {
                                            self.selectedSholarshipId = scholarship.id
                                            self.showScholarshipDetailView.toggle()
                                        }
                                } else {
                                    SholarshipCard(width: geometry.size.width-32, scholarship: scholarship)
                                        .onTapGesture {
                                            self.selectedSholarshipId = scholarship.id
                                            self.showScholarshipDetailView.toggle()
                                        }
            ...
            .task {
                await Task { @MainActor in
                    viewModel.getSaucierScholarshipPosts()
                }.value
            }
            ...
            .navigationDestination(isPresented: $showScholarshipDetailView) {
                if showScholarshipDetailView {
                    ScholarshipDetailView(scholarshipId: selectedSholarshipId)
                }
            }
        }
    }
